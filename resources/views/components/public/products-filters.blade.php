<form action="{{ route('search') }}" class="mb-3" id="filters">
  ...
  <div class="d-flex gap-2">
    ...
    <div class="dropdown">
      <button type="button" @class($filterBtnCls . 'dropdown-toggle"' ) id="dropdown-price-limit" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        Price limit {{ request('filter.price_limit') }}
      </button>
      <div class="dropdown-menu shadow" aria-labelledby="dropdown-price-limit">
        <div class="input-group">
          <input type="number" class="form-control" placeholder="Price limit" aria-describedby="btnGroupAddon" name="filter[price_limit]" value="{{ request('filter.price_limit') }}">
